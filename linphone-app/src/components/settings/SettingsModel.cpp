...
SettingsModel::SettingsModel(QObject *parent) : QObject(parent) {
	...
	connect(this, &SettingsModel::dontAskAgainInfoEncryptionChanged, this,
	        &SettingsModel::haveDontAskAgainChoicesChanged);
	connect(this, &SettingsModel::haveAtLeastOneVideoCodecChanged, this, &SettingsModel::videoAvailableChanged);

	QObject::connect(coreManager->getHandlers().get(), &CoreHandlers::callCreated, this,
	                 &SettingsModel::handleCallCreated);
	QObject::connect(coreManager->getHandlers().get(), &CoreHandlers::callStateChanged, this,
	                 &SettingsModel::handleCallStateChanged);
	QObject::connect(coreManager->getHandlers().get(), &CoreHandlers::ecCalibrationResult, this,
	                 &SettingsModel::handleEcCalibrationResult);

	// Readonly state that can change from default account
	connect(coreManager->getAccountSettingsModel(), &AccountSettingsModel::defaultAccountChanged, this,
	        &SettingsModel::groupChatEnabledChanged);
	connect(coreManager->getAccountSettingsModel(), &AccountSettingsModel::defaultAccountChanged, this,
	        &SettingsModel::videoConferenceEnabledChanged);
	connect(coreManager->getAccountSettingsModel(), &AccountSettingsModel::defaultAccountChanged, this,
	        &SettingsModel::secureChatEnabledChanged);
	connect(coreManager->getAccountSettingsModel(), &AccountSettingsModel::defaultAccountChanged, this,
	        &SettingsModel::onDefaultAccountChanged);

	connect(coreManager->getAccountSettingsModel(), &AccountSettingsModel::accountSettingsUpdated, this,
	        &SettingsModel::groupChatEnabledChanged);
	connect(coreManager->getAccountSettingsModel(), &AccountSettingsModel::accountSettingsUpdated, this,
	        &SettingsModel::videoConferenceEnabledChanged);
	connect(coreManager->getAccountSettingsModel(), &AccountSettingsModel::accountSettingsUpdated, this,
	...
	connect(&mVfsUtils, &VfsUtils::keyDeleted, this, [&](const QString &key) {
		if (key == mVfsUtils.getApplicationVfsEncryptionKey()) {
			...
			emit vfsEncryptedChanged();
			if (mVfsUtils.needToDeleteUserData()) Utils::deleteAllUserData();
			else {
...
QString SettingsModel::getAssistantRegistrationUrl() const {
	return Utils::coreStringToAppString(
	    mConfig->getString(UiSection, "assistant_registration_url", Constants::DefaultAssistantRegistrationUrl));
}
...
QString SettingsModel::getAssistantLoginUrl() const {
	return Utils::coreStringToAppString(
	    mConfig->getString(UiSection, "assistant_login_url", Constants::DefaultAssistantLoginUrl));
}
...
QString SettingsModel::getAssistantLogoutUrl() const {
	return Utils::coreStringToAppString(
	    mConfig->getString(UiSection, "assistant_logout_url", Constants::DefaultAssistantLogoutUrl));
}
...
void SettingsModel::stopCaptureGraph() {
	if (mCaptureGraphListenerCount > 0) {
		if (--mCaptureGraphListenerCount == 0) deleteCaptureGraph();
	}
}
...
QStringList SettingsModel::getCaptureDevices() const {
	...
	for (const auto &device : core->getSoundDevicesList()) {
		if (core->soundDeviceCanCapture(device)) list << Utils::coreStringToAppString(device);
	}
	return list;
}
...
QString SettingsModel::getRingerDevice() const {
	return Utils::coreStringToAppString(CoreManager::getInstance()->getCore()->getRingerDevice());
}

void SettingsModel::setRingerDevice(const QString &device) {
	CoreManager::getInstance()->getCore()->setRingerDevice(Utils::appStringToCoreString(device));
...
void SettingsModel::setRingPath(const QString &path) {
	...
	CoreManager::getInstance()->getCore()->setRing(Utils::appStringToCoreString(cleanedPath));
...
void SettingsModel::accessVideoSettings() {
	// if(!getIsInCall())// TODO : This is a workaround to a crash when reloading video devices while in call. Spotted
	// on Macos.
	CoreManager::getInstance()->getCore()->reloadVideoDevices();
	emit videoDevicesChanged(getVideoDevices());
}
...
QString SettingsModel::getVideoDevice() const {
	return Utils::coreStringToAppString(CoreManager::getInstance()->getCore()->getVideoDevice());
}

void SettingsModel::setVideoDevice(const QString &device) {
	CoreManager::getInstance()->getCore()->setVideoDevice(Utils::appStringToCoreString(device));
...
QString SettingsModel::getVideoPreset() const {
	return Utils::coreStringToAppString(CoreManager::getInstance()->getCore()->getVideoPreset());
}

void SettingsModel::setVideoPreset(const QString &preset) {
	CoreManager::getInstance()->getCore()->setVideoPreset(Utils::appStringToCoreString(preset));
...
QVariantMap SettingsModel::getCurrentPreviewVideoDefinition() const {
	if (CoreManager::getInstance()->getCore()->videoPreviewEnabled()) {
		...
		if (definition) return createMapFromVideoDefinition(definition);
	}
	QVariantMap map;
	map["width"] = 0;
	map["height"] = 0;
...
bool SettingsModel::haveAtLeastOneVideoCodec() const {
	...
	for (auto &codec : codecs) {
		if (codec->enabled() && codec->isUsable()) return true;
	}
	return false;
}
...
SettingsModel::CameraMode SettingsModel::cameraModefromString(const std::string &mode) {
	if (mode == "Hybrid") return CameraMode::CameraMode_Hybrid;
	else if (mode == "BlackBars") return CameraMode::CameraMode_BlackBars;
	else return CameraMode::CameraMode_OccupyAllSpace;
}

std::string SettingsModel::toString(const CameraMode &mode) {
	std::string modeStr;
	switch (mode) {
		case CameraMode::CameraMode_Hybrid:
			modeStr = "Hybrid";
			break;
		case CameraMode::CameraMode_BlackBars:
			modeStr = "BlackBars";
			break;
		default:
			modeStr = "OccupyAllSpace";
	}
	return modeStr;
}
...
LinphoneEnums::ConferenceLayout SettingsModel::getVideoConferenceLayout() const {
	return (LinphoneEnums::ConferenceLayout)mConfig->getInt(UiSection, "video_conference_layout",
	                                                        (int)LinphoneEnums::ConferenceLayoutActiveSpeaker);
}
...
void SettingsModel::setStandardChatEnabled(bool status) {
	if (!isReadOnly(UiSection, "standard_chat_enabled")) mConfig->setInt(UiSection, "standard_chat_enabled", status);
...
bool SettingsModel::getSecureChatEnabled() const {
	return getChatEnabled() && !!mConfig->getInt(UiSection, getEntryFullName(UiSection, "secure_chat_enabled"), 1) &&
	       getLimeIsSupported() && CoreManager::getInstance()->getCore()->getDefaultAccount() &&
	       !CoreManager::getInstance()->getCore()->getDefaultAccount()->getParams()->getLimeServerUrl().empty()
...
void SettingsModel::setSecureChatEnabled(bool status) {
	if (!isReadOnly(UiSection, "secure_chat_enabled")) mConfig->setInt(UiSection, "secure_chat_enabled", status);
...
bool SettingsModel::getGroupChatEnabled() const {
	return CoreManager::getInstance()->getCore()->getDefaultAccount() &&
	       !CoreManager::getInstance()->getCore()->getDefaultAccount()->getParams()->getConferenceFactoryUri().empty();
}
...
bool SettingsModel::getHideEmptyChatRooms() const {
	int defaultValue = 0;
	if (!mConfig->hasEntry("misc", "hide_empty_chat_rooms")) // This step should be removed when this option comes from
	                                                         // API and not directly from config file
...
QString SettingsModel::getFileTransferUrl() const {
	return Utils::coreStringToAppString(CoreManager::getInstance()->getCore()->getFileTransferServer());
}

void SettingsModel::setFileTransferUrl(const QString &url) {
	CoreManager::getInstance()->getCore()->setFileTransferServer(Utils::appStringToCoreString(url));
...
QVariantList SettingsModel::getSupportedMediaEncryptions() const {
	...
	if (core->mediaEncryptionSupported(linphone::MediaEncryption::ZRTP)) {
		...
		else list << buildEncryptionDescription(MediaEncryptionZrtp, "ZRTP");
	}
...
SettingsModel::MediaEncryption SettingsModel::getMediaEncryption() const {
	return static_cast<SettingsModel::MediaEncryption>(CoreManager::getInstance()->getCore()->getMediaEncryption());
}

void SettingsModel::setMediaEncryption(MediaEncryption encryption) {
	if (encryption == getMediaEncryption()) return;

	CoreManager::getInstance()->getCore()->setMediaEncryption(static_cast<linphone::MediaEncryption>(encryption));
...
void SettingsModel::setContactsEnabled(bool status) {
	if (!isReadOnly(UiSection, "contacts_enabled")) mConfig->setInt(UiSection, "contacts_enabled", status);
...
void SettingsModel::setAudioPortRange(const QList<int> &range) {
	...
	if (b == -1) core->setAudioPort(a);
	else core->setAudioPortRange(a, b);
...
void SettingsModel::setVideoPortRange(const QList<int> &range) {
	...
	if (b == -1) core->setVideoPort(a);
	else core->setVideoPortRange(a, b);
...
QString SettingsModel::getStunServer() const {
	return Utils::coreStringToAppString(CoreManager::getInstance()->getCore()->getNatPolicy()->getStunServer());
}

void SettingsModel::setStunServer(const QString &stunServer) {
	CoreManager::getInstance()->getCore()->getNatPolicy()->setStunServer(Utils::appStringToCoreString(stunServer));
}

// -----------------------------------------------------------------------------

QString SettingsModel::getTurnUser() const {
	return Utils::coreStringToAppString(CoreManager::getInstance()->getCore()->getNatPolicy()->getStunServerUsername());
}

void SettingsModel::setTurnUser(const QString &user) {
	CoreManager::getInstance()->getCore()->getNatPolicy()->setStunServerUsername(Utils::appStringToCoreString(user));
...
QString SettingsModel::getTurnPassword() const {
	...
	shared_ptr<const linphone::AuthInfo> authInfo(
	    core->findAuthInfo("", natPolicy->getStunServerUsername(), natPolicy->getStunServer()));
...
void SettingsModel::setTurnPassword(const QString &password) {
	...
	if (authInfo) {
		...
		core->addAuthInfo(linphone::Factory::get()->createAuthInfo(turnUser, turnUser,
...
void SettingsModel::updateRlsUri() {
	if (getRlsUriEnabled() && getRlsUri().isEmpty()) { // if enabled, uri should not be empty : set default. This allow
		                                               // to take account of old configuration.
...
QString SettingsModel::getSavedScreenshotsFolder() const {
	...
	if (path == "") path = Paths::getCapturesDirPath();
...
static inline string getLegacySavedCallsFolder(const shared_ptr<linphone::Config> &config) {
	...
	if (path ==
	    "") // Avoid to call default function if exist because calling Path:: will create a folder to be writable.
...
QString SettingsModel::getDownloadFolder() const {
	auto path = mConfig->getString(
	    UiSection, "download_folder",
	    ""); // Avoid to call default function if exist because calling Path:: will create a folder to be writable.
	if (path == "") path = Paths::getDownloadDirPath();
	return QDir::cleanPath(Utils::coreStringToAppString(path)) + QDir::separator();
}
...
void SettingsModel::setRemoteProvisioning(const QString &remoteProvisioning) {
	...
	else emit remoteProvisioningNotChanged(urlRemoteProvisioning);
}
...
void SettingsModel::setVersionCheckUrl(const QString &url) {
	if (url != getVersionCheckUrl()) {
		...
		if (url == Constants::VersionCheckReleaseUrl) setVersionCheckType(VersionCheckType_Release);
		else if (url == Constants::VersionCheckNightlyUrl) setVersionCheckType(VersionCheckType_Nightly);
		else setVersionCheckType(VersionCheckType_Custom);
...
SettingsModel::VersionCheckType SettingsModel::getVersionCheckType() const {
	return (SettingsModel::VersionCheckType)mConfig->getInt(UiSection, "version_check_type",
	                                                        (int)VersionCheckType_Release);
}
	...
	if (type != getVersionCheckType()) {
		...
		switch (type) {
			case VersionCheckType_Release:
				setVersionCheckUrl(Constants::VersionCheckReleaseUrl);
				break;
			case VersionCheckType_Nightly:
				setVersionCheckUrl(Constants::VersionCheckNightlyUrl);
				break;
			case VersionCheckType_Custom:
				break; // Do not override URL
		}
...
Utils::SipDisplayMode SettingsModel::getSipDisplayMode() const {
	return static_cast<Utils::SipDisplayMode>(mConfig->getInt(
	    UiSection, getEntryFullName(UiSection, "sip_display_mode"), (int)Utils::SipDisplayMode::SIP_DISPLAY_ALL));
}
...
void SettingsModel::setLogsFolder(const QString &folder) {
	...
	CoreManager::getInstance()->getCore()->setLogCollectionPath(
	    logPath); // Update Core to the new path. Liblinphone should update it.
...
QString SettingsModel::getLogsUploadUrl() const {
	return Utils::coreStringToAppString(CoreManager::getInstance()->getCore()->getLogCollectionUploadServerUrl());
}

void SettingsModel::setLogsUploadUrl(const QString &url) {
	CoreManager::getInstance()->getCore()->setLogCollectionUploadServerUrl(Utils::appStringToCoreString(url));
...
QString SettingsModel::getOAuth2Password() const {
	return Utils::coreStringToAppString(mConfig->getString(UiSection, "oauth2_password", Constants::OAuth2Password));
}

QString SettingsModel::getOAuth2Scope() const {
	return Utils::coreStringToAppString(mConfig->getString(UiSection, "oauth2_scope", Constants::OAuth2Scope));
}
...
QString SettingsModel::getOAuth2RemoteProvisioningHeader() const {
	return Utils::coreStringToAppString(mConfig->getString(UiSection, "oauth2_remote_provisioning_header",
	                                                       Constants::DefaultOAuth2RemoteProvisioningHeader));
}

// ---------------------------------------------------------------------------

QString SettingsModel::getLogsFolder(const shared_ptr<linphone::Config> &config) {
	return Utils::coreStringToAppString(config ? config->getString(UiSection, "logs_folder", Paths::getLogsDirPath())
	                                           : Paths::getLogsDirPath());
}

size_t SettingsModel::getMaxLogsCollectionSize(const shared_ptr<linphone::Config> &config) {
	return config ? config->getInt(UiSection, "logs_max_size", Constants::MaxLogsCollectionSize)
...
void SettingsModel::onDefaultAccountChanged() {
	mConfig->setInt("misc", "hide_chat_rooms_from_removed_proxies",
	                CoreManager::getInstance()->getCore()->getDefaultAccount() != nullptr);
}
